...
export default function Shop() {
  // ── Read ?category= from the URL on first load (e.g. arriving from the ──
  // ── homepage "stories" carousel) and keep the URL in sync afterwards so ──
  // ── the filtered view is shareable/bookmarkable and survives a refresh. ──
  const [searchParams, setSearchParams] = useSearchParams()
  const categoryFromUrl = searchParams.get('category')

  const [activeCategory, setActiveCategory] = useState<ProductCategory | 'all'>(
    (categoryFromUrl as ProductCategory) ?? 'all',
  )
  const [selectedProduct, setSelectedProduct] = useState<Product | null>(null)

  // If the URL's category changes after mount (e.g. user clicks a different
  // homepage tile and React Router re-renders this same page instance),
  // pick that up too.
  useEffect(() => {
    if (categoryFromUrl && categoryFromUrl !== activeCategory) {
      setActiveCategory(categoryFromUrl as ProductCategory)
  ...
  // Keep the URL in sync when the user changes the filter manually via
  // CategoryFilter — so the page stays shareable/bookmarkable.
